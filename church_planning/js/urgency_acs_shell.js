...
(function (global) {
  ...
  function showToast(msg) {
    ...
    setTimeout(function () { el.classList.remove("urgency-toast--show"); }, 3200);
  }
  ...
  function renderSurvey() {
    ...
    var html =
      ...
      '<p class="text-sm text-slate-600 mb-3">約 15 分鐘 · 非效率競賽 · 完成後前往【3. 四象限儀表】。</p>' +
    ...
    html +=
      ...
      '<button type="submit" class="acs-btn acs-btn--primary mt-3 w-full py-3">✓ 提交 → 四象限儀表</button></form>';
    host.innerHTML = html;
  }
  ...
  function renderFlags(run) {
    ...
    if (!flags.length) {
      zone.innerHTML = "<p>目前無明顯象限預警；仍建議與導師確認 Q2 固定時段。</p>";
      return;
    }
    zone.innerHTML =
  ...
  function renderReport(run, opts) {
    ...
    if (banner) {
      if (run.is_demo) {
        banner.innerHTML = '<p class="text-sm text-amber-900"><strong>示範模式</strong>：Q1 偏高 · Q2 偏低 · 橫條將展開</p>';
    ...
    if (summary && run.derived) {
      ...
      summary.innerHTML =
        ...
        d.q1_pct +
        "%</p>";
      if ((run.risk_flags || []).indexOf("Q2_BELOW_TARGET") >= 0) {
        summary.innerHTML +=
          '<p class="text-rose-800 font-bold mt-2">⚠️ Q2 偏低：請開啟【4. 優先序決策桌】</p>';
      }
    }
  ...
  function loadDemoReport() {
    if (!global.UrgencyPack) { showToast("模組尚未載入"); return; }
    var built = UrgencyPack.buildDemoRun();
    if (!built.ok) { showToast("示範載入失敗"); return; }
    showToast("示範已載入 · 前往四象限儀表…");
    switchToReport();
    setTimeout(function () { renderReport(built.run, { animate: true }); }, 280);
  }
